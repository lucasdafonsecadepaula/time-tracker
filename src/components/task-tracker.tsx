...
import { getTodayTasks, saveTodayTasks, Task, getActiveTask, saveActiveTask, ActiveTask } from '@/lib/storage'
...
export const TaskTracker = ({ onTimeUpdate }: { onTimeUpdate: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (savedActiveTask) {
      ...
      setTasks((prev) =>
        prev.map((task) => (task.id === savedActiveTask.id ? { ...task, timeSpent: task.timeSpent + elapsed } : task)),
      )
  ...
  useEffect(() => {
    if (activeTask) {
      intervalRef.current = window.setInterval(() => {
        setTasks((prev) =>
          prev.map((task) => (task.id === activeTask.id ? { ...task, timeSpent: task.timeSpent + 1 } : task)),
        )
      }, 1000)
    } else {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && activeTask) {
        ...
        setTasks((prev) =>
          prev.map((task) => (task.id === activeTask.id ? { ...task, timeSpent: task.timeSpent + elapsed } : task)),
        )
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    {editingId === task.id ? (
                      ...
                      <h3 className="truncate text-lg font-semibold">{task.name}</h3>
                    )}
                    <p className="text-primary mt-1 font-mono text-2xl">{formatTime(task.timeSpent)}</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="icon" variant="outline" onClick={() => startEdit(task)}>
                      <Edit2 />
                    </Button>
          ...
          {tasks.length === 0 && (
            <p className="text-muted-foreground py-8 text-center">No tasks yet. Add one to get started!</p>
          )}
        </div>
      </CardContent>
    </Card>
